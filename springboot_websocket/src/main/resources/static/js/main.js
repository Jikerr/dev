...
function initObject(){
    //只有当菜单对象为空时候才创建 ,不然切换关卡丢失玩家人数数据等
    stage = new Stage(ctx,level);
    map = new Map(wallCtx,grassCtx);

    if(undefined == menu || null == menu || gameState==GAME_STATE_OVER) {
        menu = new Menu(ctx);
	}
    if(player1==undefined || player1==null || gameState==GAME_STATE_OVER){
        player1 = new PlayTank(tankCtx);
	}
    if(player2==undefined || player2==null || gameState==GAME_STATE_OVER){
...
$(document).keydown(function(e){
	...
	case GAME_STATE_START:
		...
		if(e.keyCode == keyboard.SPACE && player1.lives > 0){
			...
			//player2.shoot(BULLET_TYPE_ENEMY);//解决BUG PLAY2射击无法命中敌人的问题
